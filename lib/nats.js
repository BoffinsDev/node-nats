...
Client.prototype.setupHandlers = function() {
  ...
  function processInbound(time, count) {
    if (client.closed || !client.inbound || client.inbound.length == 0) {
      return;
    }
    ...
    if (time - Date.now() > 5 || count > 8) {
      setTimeout(function() {
        processInbound(Date.now(), 0);
      }, 0);
    } else {
      processInbound(time, count + 1);
    }
  }
...
Client.prototype.createConnection = function() {
  ...
  this.selectServer();
  // Create the stream.
  this.stream = net.createConnection(this.options.url);
...
Client.prototype.processInbound = function() {
  ...
  while (!client.closed && client.inbound && client.inbound.length > 0) {
	...
	if ((Date.now() - start) > client.options.yieldTime) {
	  client.stream.pause();
	  setTimeout(client.processInbound.bind(this), 0);
	  return;
	}
      }
      break;
